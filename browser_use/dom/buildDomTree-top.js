(
    ...
    function isTopElement(element) {
        ...
        if (doc !== window.document) {
            return isTopElementInFrame(element);
        }
        
        // Handle main document elements
        return isTopElementInMainDocument(element);
    }

    function isTopElementInFrame(element) {
        const rect = element.getBoundingClientRect();
        const frameDoc = element.ownerDocument;
        
        // Check if element is visible within iframe bounds
        if (rect.width === 0 || rect.height === 0) {
            return false;
        }
        
        // Use iframe's document for elementFromPoint
        const centerX = rect.left + rect.width / 2;
        const centerY = rect.top + rect.height / 2;
        
        // Check if point is within iframe viewport
        const frameWindow = frameDoc.defaultView;
        if (centerX < 0 || centerX >= frameWindow.innerWidth || 
            centerY < 0 || centerY >= frameWindow.innerHeight) {
            return false;
        }
        
        try {
            const topEl = frameDoc.elementFromPoint(centerX, centerY);
            if (!topEl) return false;
            
            // Check if found element is our target or contains it
            let current = topEl;
            while (current && current !== frameDoc.documentElement) {
    ...
    function isTopElementInMainDocument(element) {
        ...
        try {
            ...
            if (point.x < 0 || point.x >= window.innerWidth || 
                ...
                return false; // Elements with center outside viewport are NOT visible
            }
